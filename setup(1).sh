#!/bin/bash
# SentinelAI v1.4 Kurulum Betiği
...
EOF
echo -e "${R}"
echo -e "  ${GRI}SentinelAI v1.4 Kurulum Betiği  |  Yazar: Mehmed Gürbüz (s247003009)${R}"
echo -e "  ${GRI}──────────────────────────────────────────────────────${R}\n"

KURULUM_DIR="$HOME/sentinelai"

# ── 1. Python kontrolü ───────────────────────────────────────
echo -e "  ${CYAN}[1/5] Python kontrol ediliyor...${R}"
PYTHON=""
...
# ── 2. Google Generative AI kütüphanesi ──────────────────────
echo -e "\n  ${CYAN}[2/5] Google Generative AI kütüphanesi kontrol ediliyor...${R}"
if $PYTHON -c "import google.generativeai" &>/dev/null; then
    echo -e "  ${YESIL}✅ google-generativeai zaten kurulu.${R}"
else
    echo -e "  ${GRI}google-generativeai bulunamadı.${R}"
    read -p "$(echo -e "  ${SARI}Kurulsun mu? (pip --user) [E/H]: ${R}")" CEVAP
    if [[ "$CEVAP" =~ ^[Ee]$ ]]; then
        pip install --user google-generativeai
        echo -e "  ${YESIL}✅ Kuruldu.${R}"
    else
        echo -e "  ${GRI}⏭  Atlandı. AI modülü çalışmayabilir.${R}"
    fi
fi

# ── 3. Proje dosyaları ───────────────────────────────────────
echo -e "\n  ${CYAN}[3/5] Proje dosyaları indiriliyor...${R}"
...
if [ -d "$KURULUM_DIR" ]; then
    ...
    git clone https://github.com/mehmedgur/sentinelaiv1.4.git "$KURULUM_DIR"
    if [ $? -eq 0 ]; then
...
# ── 5. İzinler ve alias ──────────────────────────────────────
echo -e "  ${CYAN}[5/5] Ayarlar yapılandırılıyor...${R}"

chmod +x "$KURULUM_DIR/sentinelai.py"
echo -e "  ${YESIL}✅ Çalıştırma izni verildi.${R}"

# Dizin izinleri — sadece root erişebilir
sudo chown root:root "$KURULUM_DIR/logs" 2>/dev/null || mkdir -p "$KURULUM_DIR/logs" && sudo chown root:root "$KURULUM_DIR/logs"
sudo chown root:root "$KURULUM_DIR/data" 2>/dev/null || mkdir -p "$KURULUM_DIR/data" && sudo chown root:root "$KURULUM_DIR/data"
sudo chmod 700 "$KURULUM_DIR/logs"
sudo chmod 700 "$KURULUM_DIR/data"
echo -e "  ${YESIL}✅ logs/ ve data/ dizinleri root korumasına alındı.${R}"

# Config dosyası izni
if [ -f "$HOME/.sentinelai.conf" ]; then
    sudo chown root:root "$HOME/.sentinelai.conf"
    sudo chmod 600 "$HOME/.sentinelai.conf"
    echo -e "  ${YESIL}✅ .sentinelai.conf root korumasına alındı.${R}"
fi
...
if ! grep -q "alias sentinelai=" "$HOME/.bashrc" 2>/dev/null; then
    echo "" >> "$HOME/.bashrc"
    echo "# SentinelAI v1.4" >> "$HOME/.bashrc"
